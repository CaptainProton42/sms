...
void TEMario::load(JSUMemoryInputStream& pStream)
{
	TSpineEnemy::load(pStream);

	pStream.read(&unk154, 4);
	pStream.read(&unk158, 4);
	pStream.read(&unk15C, 4);
	pStream.read(&unk160, 4);
	...
	mEnemyMario->unk118 |= MARIO_FLAG_NPC_TALKING;
}
...
void TEMario::init(TLiveManager* manager)
{
	...
	if (mMActor != nullptr) {
		gpScreenTexture->replace(mMActor->unk4->mModelData,
		                         "H_kagemario_dummy");
	}
...
BOOL TEMario::receiveMessage(THitActor* pSender, u32 pMessage)
{
	TSpineEnemy::receiveMessage(pSender, pMessage);

	if (pMessage == HIT_MESSAGE_SPRAYED_BY_WATER) {
		mEnemyMario->hitWater(pSender);
		return TRUE;
	} else if (pMessage == HIT_MESSAGE_TRAMPLE) {
		return mEnemyMario->thinkTrample();
	} else if (pSender->getActorType() == 0x40000246) {
...
void TEMario::startMonteReplay(u32 pParam1)
{
	mEnemyMario->startMonteReplay(pParam1);
}
...
void TEMario::perform(u32 pFlags, JDrama::TGraphics* pGfx)
{
	...
	mEnemyMario->perform(pFlags, pGfx);
...
TEMarioManager::TEMarioManager(const char* pName)
    : TEnemyManager(pName)
{
}

void TEMarioManager::load(JSUMemoryInputStream& pStream)
{
	unk38 = new TSpineEnemyParams("/enemy/emario.prm");
	TEnemyManager::load(pStream);
}
